...
class MorphResnet(nn.Module):

    def __init__(self, block, layers, cbam=False, dcn=False):
        super(MorphResnet, self).__init__()
        self.inplanes = 64
        self.dcn = dcn
        self.cbam = cbam
        self.conv1 = nn.Conv2d(3, 64, kernel_size=7, stride=2, padding=3, bias=False)
        ...
        self.out_shape = {'C3_size': self.out_channels[0] * 2 + self.m_channels_out['m3'],
                          'C4_size': self.out_channels[1] * 2 + self.m_channels_out['m4'],
                          'C5_size': self.out_channels[2] * 2 + self.m_channels_out['m5']}

        print("backbone output channel: C3 {}, C4 {}, C5 {}".format(self.out_shape['C3_size'] , self.out_shape['C4_size'] , self.out_shape['C5_size'] ))
    ...
    def forward(self, inputs):
        ...
        x1 = self.layer1(x) 
        x2 = self.layer2(x1) # out1
        x3 = self.layer3(x2) # out2
        x4 = self.layer4(x3) # out3

        #concate
        o1 = torch.cat((x2, mc3), dim=1)
        o2 = torch.cat((x3, mc4), dim=1)
        o3 = torch.cat((x4, mc5), dim=1)
        return o1, o2, o3
...
def Morresnet18(pretrained=False, **kwargs):
    """Constructs a ResNet-18 model.
    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    model = MorphResnet(BasicBlock, [2, 2, 2, 2], **kwargs)
    if pretrained:
        model.load_state_dict(model_zoo.load_url(model_urls['resnet18'], model_dir='.'), strict=False)
    return model
...
def morresnet(pretrained=False, **kwargs):
    version = str(kwargs.pop('version'))
    if version == '18':
        return Morresnet18(pretrained, **kwargs)
